...
from management_app.models import CustomUser, Staffs, Courses, Subjects, Admission, SessionYearModel, Attendance, AttendanceReport, LeaveReportStaff, FeedBackStaffs, StudentResult


def staff_home(request):
    # Fetching All Admission under Staff
    ...
    students_count = Admission.objects.filter(course_id__in=final_course).count()
    ...
    students_attendance = Admission.objects.filter(course_id__in=final_course)
    student_list = []
...
def save_attendance_data(request):
    ...
    try:
        ...
        for stud in json_student:
            ...
            student = Admission.objects.get(admin=stud['id'])
...
def staff_add_result_save(request):
    if request.method != "POST":
        ...
        student_obj = Admission.objects.get(admin=student_admin_id)
        subject_obj = Subjects.objects.get(id=subject_id)

        try:
            # Check if Admission Result Already Exists or not
